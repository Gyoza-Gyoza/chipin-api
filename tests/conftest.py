...
from data.receipts import ReceiptCreate, ReceiptUpdate
...
def test_receipt(test_item, created_user):
    return ReceiptCreate(owner_id = created_user['user_id'],
...
def created_receipt(test_receipt):
    ...
    receiptData = client.post(
        "/receipts",
        json = receipt).json()

    response = client.get(f"/receipts/{receiptData['receipt_id']}")

    full_receipt = response.json()
